...
class BertAttention(nn.Module):
    ...
    def forward(self, graph):
        ...
        attention_output = self.output(self_output_graph.ndata['h'],
                                       input_tensor)
...
if __name__ == '__main__':
    ...
    class_list = [
        x.strip()
        for x in open(os.path.join(data_dir, "class.txt")).readlines()
    ]
    ...
    g1.ndata['input_ids'] = torch.tensor(input_ids,
                                         dtype=torch.long,
                                         device=device)
    g1.ndata['segment_ids'] = torch.tensor(segment_ids,
                                           dtype=torch.long,
                                           device=device)
    g1.ndata['position_ids'] = torch.arange(len(input_ids),
                                            dtype=torch.long,
                                            device=device)
    for i in range(seq_length):
        g1.add_edges(i, range(seq_length))

    Classification = BertForSequenceClassification.from_pretrained(
        r'./myfinetun-bert_chinese/')
    Classification.eval()
