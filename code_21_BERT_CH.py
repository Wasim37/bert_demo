...
from transformers import (get_linear_schedule_with_warmup, BertTokenizer,
                          AdamW, AutoModelForSequenceClassification,
                          AutoConfig)
...
class_list = [
    x.strip() for x in open(os.path.join(data_dir, "class.txt")).readlines()
]

pretrained_weights = 'bert-base-chinese'  #建立模型
tokenizer = BertTokenizer.from_pretrained(pretrained_weights)
config = AutoConfig.from_pretrained(pretrained_weights,
                                    num_labels=len(class_list))
#单独指定config，在config中指定分类个数
nlp_classif = AutoModelForSequenceClassification.from_pretrained(
...
def train(model, traindataloder, testdataloder):
    ...
    scheduler = get_linear_schedule_with_warmup(
        optimizer,
        num_warmup_steps=0,
        num_training_steps=len(traindataloder) * epochs)
