...
from transformers import (GPT2LMHeadModel, GPT2Config, CONFIG_NAME, AdamW,
                          BertTokenizer)
import json
import os
import torch
...
class DialogDataset(Dataset):
    ...
    def process(self, history, resposne, with_eos=True):
        ...
        sequence = [[self.bos]] + history + [
            resposne + ([self.eos] if with_eos else [])
        ]
        sequence = [sequence[0]
                    ] + [[self.speaker2 if i % 2 else self.speaker1] + s
                         for i, s in enumerate(sequence[1:])]
        instance = {}
        instance["input_ids"] = list(chain(*sequence))
        instance["token_type_ids"] = [self.bos] + [
            self.speaker2 if i % 2 else self.speaker1
            for i, s in enumerate(sequence[1:]) for _ in s
        ]
...
tokenizer = BertTokenizer(os.path.join(model_checkpoint, "vocab.txt"),
                          do_lower_case=True)
...
token_type_idssz = tokenizer.convert_ids_to_tokens(
    valid_dataset[0]['token_type_ids'])
...
config = GPT2Config.from_json_file(
    os.path.join(model_checkpoint, 'config.json'))
    ...
    for batch in valid_loader:
        with torch.no_grad():
            input_ids, token_type_ids, lm_labels = tuple(
                input_tensor.to(device) for input_tensor in batch)
            lm_logits, *_ = model(input_ids, token_type_ids=token_type_ids)
            lm_logits_flat_shifted = lm_logits[..., :-1, :].reshape(
                -1, lm_logits.size(-1))
...
for epoch in range(max_epochs):
    ...
    evalloss = evaltest()
    print('train:', np.mean(allloss), "lr", optimizer.param_groups[0]['lr'],
          'eval', evalloss)
    torch.save({'state_dict': model.cpu().state_dict()},
               os.path.join(logdir, 'model_training.bin'))
