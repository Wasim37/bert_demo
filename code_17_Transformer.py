...
# source (china)
ZH = data.Field(init_token='<sos>',
                eos_token='<eos>',
                lower=True,
                batch_first=True)
# target (English)
ENG = data.Field(init_token='<sos>',
                 eos_token='<eos>',
                 lower=True,
                 batch_first=True)
...
valid_iterator = data.BucketIterator(dataset=valid_data,
                                     shuffle=False,
                                     batch_size=BATCH_SIZE,
                                     device=device)

test_iterator = data.BucketIterator(dataset=test_data,
                                    shuffle=False,
                                    batch_size=BATCH_SIZE,
                                    device=device)
...
class MultiHeadAttention(nn.Module):
    def __init__(self, hidden_size, n_head, device, dropout=0.1):
        ...
        self.scaled_dot_attention = ScaledDotProductAttention(
            torch.sqrt(torch.FloatTensor([hidden_size // n_head])).to(device))
    ...
    def forward(self, q, k, v, mask=None):
        ...
        # Project and split
        q = self.w_q(q).view(
            batch_size, -1, self.n_head, self.hidden_size //
            self.n_head)  # [B, T, H] -> [B, T, n_head, H//n_head]
        k = self.w_k(k).view(
            batch_size, -1, self.n_head, self.hidden_size //
            self.n_head)  # [B, T, H] -> [B, T, n_head, H//n_head]
        v = self.w_v(v).view(
        ...
        output, attn = self.scaled_dot_attention(
            q, k, v, mask)  # [B, n_head, T, T], [B, n_head, T, H//n_head]
        output = output.transpose(1,
                                  2).contiguous()  # [B, T, n_head, H//n_head]
        output = output.view(batch_size, -1,
                             self.n_head *
                             (self.hidden_size // self.n_head))  # [B, T, H]
...
class PositionwiseFeedForward(nn.Module):
    def __init__(self, hidden_size, filter_size, dropout):
        ...
        self.pff = nn.Sequential(nn.Linear(hidden_size, filter_size),
...
class EncoderLayer(nn.Module):
    def __init__(self, hidden_size, filter_size, n_head, pre_lnorm, device,
                 dropout):
    ...
    def forward(self, src, src_mask):
        if self.pre_lnorm:
            ...
            src = src + self.self_attn(pre, pre, pre,
                                       src_mask)  # residual connection

            pre = self.pff_norm(src)
            src = src + self.pff(pre)  # residual connection
        else:
            src = self.self_attn_norm(src + self.self_attn(
                src, src, src, src_mask))  # residual connection + layerNorm
            src = self.pff_norm(
                src + self.pff(src))  # residual connection + layerNorm
...
class PositionalEmbedding(nn.Module):
    def __init__(self, embed_size):
        ...
        inv_timescales = 1 / (10000**(torch.arange(0.0, embed_size, 2.0) /
                                      embed_size))
...
class Encoder(nn.Module):
    def __init__(self, input_size, hidden_size, filter_size, n_head, dropout,
                 n_layers, pre_lnorm, device):
        ...
        self.wpe = nn.Embedding.from_pretrained(positional_encoding_table(
            max_len + 1, hidden_size, padding_idx=ZH.vocab.stoi['<pad>']),
                                                freeze=True)
        self.embed_dropout = nn.Dropout(dropout)
        self.layers = nn.ModuleList([
            EncoderLayer(hidden_size, filter_size, n_head, pre_lnorm, device,
    ...
    def forward(self, src, src_mask):
        ...
        pos = torch.arange(0,
                           src.shape[1]).unsqueeze(0).repeat(src.shape[0],
                                                             1).to(self.device)
...
class DecoderLayer(nn.Module):
    ...
    def forward(self, enc_out, enc_out_mask, trg, trg_mask):
        if self.pre_lnorm:
            ...
            trg = self.self_attn_norm(trg +
                                      self.self_attn(trg, trg, trg, trg_mask))
...
class Decoder(nn.Module):
    def __init__(self, input_size, hidden_size, filter_size, n_head, dropout,
        ...
        max_len = 1000
        self.wpe = nn.Embedding.from_pretrained(positional_encoding_table(
            max_len + 1, hidden_size, padding_idx=ENG.vocab.stoi['<pad>']),
                                                freeze=True)
        self.embed_dropout = nn.Dropout(dropout)
        self.layers = nn.ModuleList([
            DecoderLayer(hidden_size, filter_size, n_head, pre_lnorm, device,
                         dropout) for _ in range(n_layers)
        ])
        self.pre_lnorm = pre_lnorm
        self.last_norm = nn.LayerNorm(hidden_size)
    ...
    def forward(self, enc_out, enc_out_mask, trg, trg_mask):
        ...
        pos = torch.arange(0,
                           trg.shape[1]).unsqueeze(0).repeat(trg.shape[0],
                                                             1).to(self.device)
...
class Transformer(nn.Module):
    def __init__(self,
        ...
        self.encoder = Encoder(enc_input_size, hidden_size, filter_size,
                               n_head, dropout, n_layers, pre_lnorm, device)
        self.decoder = Decoder(dec_input_size, hidden_size, filter_size,
                               n_head, dropout, n_layers, pre_lnorm, device)
    ...
    def inference(self, src):
        ...
        for t in range(self.maxlen - 1):
            dec_out = self.decoder(enc_out, src_mask, trg,
                                   trg_mask)  # [B, T, H]
...
model = Transformer(enc_input_size, dec_input_size, hidden_size, filter_size,
                    n_head, dropout, n_layers, pre_lnorm, device)
...
plt.plot(np.arange(1, total_step), [[opt.learning_rate(i) for opt in opts]
...
if pre_lnorm:
    ...
    optimizer = NoamOpt(
        optim.Adam(model.parameters(), lr=0, betas=betas, eps=eps),
        hidden_size, factor, warmup)
...
def train(model, iterator, optimizer, criterion):
    ...
    for batch in tqdm(iterator):
        ...
        output = model(src, src_mask, trg[:, :-1],
                       trg_mask)  # [B, T-1, output_size]

        output = output.contiguous().view(
            -1, output.shape[-1])  # [B*(T-1), output_size]
...
for epoch in range(n_epoch):
    ...
    print(
        f'Epoch: {epoch+1:02} | Train Loss: {train_loss:.3f} | Validation Loss: {valid_loss:.3f} | Bleu Score: {bleu_value:.2f} | Since: {hour}h {minute}m {second:.0f}s'
    )
...
plt.plot(epoch_record,
         valid_loss_record_post,
         'x--',
         label='Post-LN (Adam w/ warm-up)')
plt.plot(epoch_record,
         valid_loss_record_pre,
         '.-',
         color="red",
         label='Pre-LN (Adam w/o warm-up)')
# plt.title('')
...
plt.plot(epoch_record,
         bleu_score_record_post,
         'x--',
         label='Post-LN (Adam w/ warm-up)')
plt.plot(epoch_record,
         bleu_score_record_pre,
         '.-',
         color="red",
         label='Pre-LN (Adam w/o warm-up)')
# plt.title('')
