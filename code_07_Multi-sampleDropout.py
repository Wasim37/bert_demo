...
class Logic_TDropout_Net(LogicNet):
    def __init__(self,
                 ...
                 outputdim,
                 dropout_num=8,
                 dropout_p=0.5):  #初始化网络结构
        super(Logic_TDropout_Net, self).__init__(inputdim, hiddendim,
                                                 outputdim)

        self.dropouts = nn.ModuleList(
            [nn.Dropout(dropout_p, inplace=False) for _ in range(dropout_num)])
...
model = Logic_TDropout_Net(inputdim=2,
                           hiddendim=500,
                           outputdim=2,
                           dropout_num=8,
                           dropout_p=0.1)  #初始化模型
