...
class AttLSTMModel(nn.Module):
    def __init__(self, vocab_size, embedding_dim, in_times, dropout, pad_idx,
                 hidden_dim, n_layer, n_class):
...
class AttentionSeq(torch.nn.Module):
    ...
    def forward(self, features, mean=False):
        ...
        if self.hard != 0:  #hard mode
            weight = torch.where(weight > self.hard, weight,
                                 torch.full_like(weight, 0))
...
if __name__ == '__main__':
    ...
    model = AttLSTMModel(INPUT_DIM, EMBEDDING_DIM, in_times, DROPOUT, PAD_IDX,
                         256, 2, OUTPUT_DIM)
    ...
    from ranger import *
    opt_func = partial(Ranger, betas=(.9, 0.99),
                       eps=1e-6)  #betas=(Momentum,alpha)
