...
LastEditTime: 2021-09-07 15:59:41
LastEditors: your name
Description: In User Settings Edit
FilePath: \bert-shizhan\code_25_BERT_NoPUNC.py
'''
# -*- coding: utf-8 -*-
"""
...
def clean_and_replace_target_name(row):  #去掉标点符号
    ...
    text = re.sub(str(A),
                  tokenizer.tokenize(A)[0], text)  #将名称之换做一个词Bob Suter--》bob
...
def savepkl(df, prename=''):
    ...
    for _, row in tqdm(df.iterrows(), total=len(df)):
        ...
        row.loc['TextClean'] = insert_tag(row,
                                          hasbrack=False)  #插入标签,防止去标点时，一起被去掉
        text = clean_and_replace_target_name(row)  #去除标点、空格，并压缩被指带的名词

        encode_rel = tokenizer.encode_plus(
            text, max_length=max_len, pad_to_max_length=True)  #向量化  len=90

        tokens, offsets, masks = tokenize(
            encode_rel['input_ids'], tokenizer,
            encode_rel['attention_mask'])  #获取标签偏移
        ...
        #输入BERT模型
        bert_outputs, bert_last_outputs = model(
            token_tensor,
            attention_mask=masks_tensor)  #[1, 107, 768] , [1, 768]
    ...
    pickle.dump(bert_prediction,
                open(prename + 'bert_outputs_forNoPUNC.pkl', "wb"))
