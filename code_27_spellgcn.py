...
class spellgcnBert(nn.Module):
    ...
    def getgnnemb(self):
        feat = self.MLbert.bert.embeddings.word_embeddings(
            n2w)  #( input_ids=torch.tensor([n2w]).to(device) )
        node_embedding = self.gnnmodel(g, feat)  #[4755, 768]
        expanded_node_embedding = node_embedding[w2n]  #21128, 768
        rest_embedding = self.MLbert.bert.get_input_embeddings(
        ).weight  #21128, 768]
        gcn_embedding = maskbase * expanded_node_embedding + (
            1 - maskbase) * rest_embedding
        return gcn_embedding

    def forward(self, input_ids, input_mask, segment_ids):
        gcn_embedding = self.getgnnemb()
        outputs = self.MLbert.bert(input_ids, input_mask,
                                   segment_ids)  #prob [batch_size, seq_len, 1]
        sequence_output = outputs[0]
        hidden_states = self.MLbert.cls.predictions.transform(sequence_output)
        prediction_scores = F.linear(hidden_states, gcn_embedding,
                                     MLbert.cls.predictions.bias)
