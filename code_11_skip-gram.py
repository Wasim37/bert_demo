...
def fenci(training_data):
    ...
    training_ci = np.reshape(training_ci, [
        -1,
    ])
    return training_ci
...
print('Sample data', training_label[:10],
      [words[i] for i in training_label[:10]])
...
class SkipGramDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        pos_indices = list(range(idx - C, idx)) + list(
            range(idx + 1, idx + 1 + C))
        pos_words = self.text_encoded[pos_indices]
        #多项式分布采样，取出指定个数的高频词
        neg_words = torch.multinomial(self.word_freqs, num_sampled + 2 * C,
                                      False)  #True)
        #去掉正向标签
        neg_words = torch.Tensor(
            np.setdiff1d(neg_words.numpy(),
                         pos_words.numpy())[:num_sampled]).long()
...
print(center_word[0], words[np.long(center_word[0])],
      [words[i] for i in pos_words[0].numpy()])
...
class Model(nn.Module):
    ...
    def forward(self, input_labels, pos_labels, neg_labels):
        ...
        log_pos = torch.bmm(pos_embedding,
                            input_embedding.unsqueeze(2)).squeeze()
        log_neg = torch.bmm(neg_embedding,
                            -input_embedding.unsqueeze(2)).squeeze()
...
valid_examples = np.random.choice(
    int(valid_window), valid_size,
    replace=False)  #0- words_size/2,中的数取16个。不能重复。
...
for e in range(NUM_EPOCHS):
    ...
    if e % 40 == 0:
        norm = torch.sum(model.in_embed.weight.data.pow(2),
                         -1).sqrt().unsqueeze(1)
