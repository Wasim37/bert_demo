...
def getmodel():
    ...
    #添加特殊词
    special_tokens_dict = {
        'additional_special_tokens': ["[THISISA]", "[THISISB]", "[THISISP]"]
    }
...
def insert_tag(
    ...
    to_be_inserted = sorted([(row["A-offset"], orgtag[0]),
                             (row["B-offset"], orgtag[1]),
                             (row["Pronoun-offset"], orgtag[2])],
                            key=lambda x: x[0],
                            reverse=True)
...
def tokenize(sequence_ind, tokenizer, sequence_mask=None):  #将标签分离，并返回标签偏移位置
    ...
    return final_tokens, (entries["[THISISA]"], entries["[THISISB]"],
                          entries["[THISISP]"]), final_mask
